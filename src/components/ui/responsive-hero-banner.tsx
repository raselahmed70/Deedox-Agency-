...
import { Menu, X, ArrowUpRight, Play, ArrowRight } from 'lucide-react';
            ...
            <header className="z-20 xl:top-4 relative">
                <div className="mx-6">
                    <div className="flex items-center justify-between pt-4">
                        <a
                            href="#"
                            className="inline-flex items-center justify-center bg-center w-[100px] h-[40px] bg-contain bg-no-repeat"
                ...
                {/* Mobile Menu */}
                {mobileMenuOpen && (
                    <div className="md:hidden absolute top-20 left-6 right-6 bg-black/80 backdrop-blur-xl rounded-2xl p-6 ring-1 ring-white/10 z-50 animate-fade-slide-in-1">
                        <div className="flex flex-col gap-4">
                            {navLinks.map((link, index) => (
                                <a
                                    key={index}
                                    href={link.href}
                                    className={`text-lg font-medium ${link.isActive ? 'text-white' : 'text-white/60'}`}
                            ...
                            <a
                                href={ctaButtonHref}
                                className="mt-2 inline-flex items-center justify-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-semibold text-neutral-900"
                                onClick={() => setMobileMenuOpen(false)}
                            >
                                {ctaButtonText}
                                <ArrowUpRight className="h-4 w-4" />
