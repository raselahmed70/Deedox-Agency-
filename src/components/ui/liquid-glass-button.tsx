...
const MetalButton = React.forwardRef<
  HTMLButtonElement,
  React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: keyof typeof colorVariants }
>(({ children, className, variant = "default", ...props }, ref) => {
  ...
  return (
    <div className={cn("relative inline-flex transform-gpu rounded-md p-[1.25px]", colors.outer)} style={wrapperStyle}>
      <div className={cn("absolute inset-[1px] transform-gpu rounded-lg", colors.inner)} style={{ transition: transitionStyle, filter: isHovered && !isPressed ? "brightness(1.05)" : "none" }} />
      <button
        ref={ref}
        className={cn("relative z-10 m-[1px] rounded-md inline-flex h-11 items-center justify-center px-6 py-2 text-sm font-semibold outline-none", colors.button, colors.textColor, colors.textShadow, className)}
